...
我们现在已决定用云供应商的数据库和大数据平台，CQRS 都是有云供应商提供的数据库前端来完成了。至于阿里云、华为云后台分布式数据库如何设计我不知道，我们只需要会 SQL 以及一些优化知识就够了，SLA 由他们保证。

X：云计算的IT基础设施真的太复杂，自己建设也是自己采购，维护与优化都是外包。虽然我们技术团队自己编程任务不多，但有效使用缓存、队列以及CQRS等知识对项目团队监管项目是有用的，是否来做个技术培训讲座？
